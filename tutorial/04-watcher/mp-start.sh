...
multipass launch --name central --cpus 1 --memory 1024M --disk 5G focal
...
multipass exec central -- sudo /bin/sh -c "curl -sfL https://get.k3s.io | K3S_URL=https://${plane_ip}:6443 K3S_TOKEN=${token} sh -"
multipass exec watcher -- sudo /bin/sh -c "curl -sfL https://get.k3s.io | K3S_URL=https://${plane_ip}:6443 K3S_TOKEN=${token} sh -"
...
kubectl label nodes watcher flussonic.com/watcher=true
